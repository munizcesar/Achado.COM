...
app.get('/api/health-telegram', async (req, res) => {
  ...
  const siteUrl = process.env.SITE_URL || 'https://achadocerto.vip';
  const checkTelegram = ['1', 'true', 'yes', 'on'].includes(String(req.query.checkTelegram || '').toLowerCase());

  const diagnostics = {
    ok: true,
    service: 'health-telegram-backend',
    timestamp: new Date().toISOString(),
    env: {
...
app.post('/api/notify-telegram', async (req, res) => {
  ...
  if (!token || !chatId) {
    return res.status(500).json({ success: false, error: 'TELEGRAM_TOKEN/telegram_token ou TELEGRAM_CHAT_ID não configurados.' });
  }

  const { title, slug, description, category, image } = req.body || {};
  if (!title || !slug) {
    return res.status(400).json({ success: false, error: 'title e slug são obrigatórios.' });
  }
  ...
  try {
    if (image) {
      const photoUrl = image.startsWith('http') ? image : `${siteUrl.replace(/\/$/, '')}${image}`;
      const sendPhotoRes = await fetch(`https://api.telegram.org/bot${token}/sendPhoto`, {
        method: 'POST',
      ...
      if (sendPhotoResult.ok) {
        return res.status(200).json({ success: true, message_id: sendPhotoResult.result.message_id });
      }
    }
    ...
    if (!sendMessageResult.ok) {
      return res.status(502).json({ success: false, error: sendMessageResult.description || 'Erro ao enviar para Telegram.' });
    }

    return res.status(200).json({ success: true, message_id: sendMessageResult.result.message_id });
  } catch (error) {
