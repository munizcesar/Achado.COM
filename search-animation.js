...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('🔍 Search Animation Script Loaded');
    console.log('Search Form:', searchForm);
    console.log('Mobile Toggle:', mobileToggle);
    
    // Toggle mobile search com animação suave
    if (mobileToggle && searchForm) {
        ...
        mobileToggle.addEventListener('click', function(e) {
            ...
            console.log('Current top:', searchForm.style.top);
            console.log('Has mobile-visible:', searchForm.classList.contains('mobile-visible'));
            
            // Verifica se está visível
            const isHidden = searchForm.style.top === '-9999px' || !searchForm.classList.contains('mobile-visible');
            
            console.log('Is hidden:', isHidden);
            
            if (isHidden) {
                console.log('🟢 MOSTRANDO formulário');
                // MOSTRAR: Traz de volta para a tela com transição suave
                searchForm.style.transition = 'opacity 0.3s ease, transform 0.3s ease';
                searchForm.style.position = 'fixed';
                searchForm.style.top = '52px';
                searchForm.style.left = '0';
                searchForm.style.right = '0';
                searchForm.style.width = '100%';
                searchForm.style.padding = '12px 20px';
                searchForm.style.background = 'rgba(11, 18, 32, 0.98)';
                searchForm.style.borderBottom = '1px solid rgba(212, 175, 55, 0.2)';
                ...
                // Force reflow para garantir que as propriedades iniciais sejam aplicadas
                searchForm.offsetHeight;
                
                // Agora anima para visível
                ...
                console.log('🔴 ESCONDENDO formulário');
                // ESCONDER: Anima para fora primeiro
                searchForm.style.transition = 'opacity 0.2s ease, transform 0.2s ease';
                searchForm.style.transform = 'translateY(-10px)';
                searchForm.style.opacity = '0';
                
                // Depois de 200ms, joga fora da tela
                setTimeout(() => {
                    searchForm.style.position = 'absolute';
                    searchForm.style.top = '-9999px';
        ...
        document.addEventListener('click', function(e) {
            if (window.innerWidth <= 768 && searchForm.classList.contains('mobile-visible')) {
                if (!e.target.closest('#main-search-form') && !e.target.closest('#mobile-search-toggle')) {
                    // Anima para fora primeiro
                    searchForm.style.transition = 'opacity 0.2s ease, transform 0.2s ease';
                    searchForm.style.transform = 'translateY(-10px)';
                    searchForm.style.opacity = '0';
                    
                    // Depois joga fora da tela
                    setTimeout(() => {
                        searchForm.style.position = 'absolute';
                        searchForm.style.top = '-9999px';
                        searchForm.style.left = '-9999px';
